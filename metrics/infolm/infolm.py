...
_CITATION = """\
@article{colombo-infolm-aaai2022,
...
_KWARGS_DESCRIPTION = """
Computes InfoLM score of a candidate text against one reference text.
Args:
...
class InfoLM(datasets.Metric):
    ...
    def _compute(self, predictions, references, idf_hyps=None, idf_ref=None, model_name="bert-base-uncased",
                 temperature=0.25, measure_to_use='fisher_rao', use_idf_weights=True, alpha=None, beta=None):
        """
        ...
        with torch.no_grad():
            dict_final_distribution_batch_refs, idfs_ref = self.get_distribution(references, idf_ref)
            dict_final_distribution_batch_hypothesis, idfs_hyp = self.get_distribution(predictions, idf_hyps)
        mask_ref = self.tokenizer(references, return_tensors="pt", padding=True, truncation=True)['input_ids']
        mask_hyps = self.tokenizer(predictions, return_tensors="pt", padding=True, truncation=True)['input_ids']
...
if __name__ == '__main__':
    ...
    infolm = datasets.load_metric("infolm")
    results = infolm.compute(predictions=predictions, references=references)
    print(results)
